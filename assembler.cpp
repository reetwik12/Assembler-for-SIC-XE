...
void modifier(unsigned int starting)
{
	...
	while(inp)
	{
		...
		if( temp.mnemonic.format==3 && temp.obcode>0xFFFFFF)
		if( temp.obcode & 0x02000000 )
		{
...
//	Printing the header record
	...
	while(inp)	
	{
		...
		if(strcmp(temp.mnemonic.name,"RESW")==0||strcmp(temp.mnemonic.name,"RESB")==0)
		{
			...
			while(strcmp(temp1.mnemonic.name,"RESW")==0||strcmp(temp1.mnemonic.name,"RESB")==0)
			{	
				temp=temp1;
				inp.read((char *)&temp1,sizeof(temp1));
				index++;
			...
			if(strcmp(temp1.mnemonic.name,"END")==0)
			{							
				break;
			}
